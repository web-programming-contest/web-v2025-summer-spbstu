...
for(let i = 0; i  < groups.length; ++i) {
    groups[i].addEventListener('click', (e) => {
            ...
            switch (grouping) {
                case "Без группировки": { //все в первый контейнер
                    ...
                    if(ingrFiltr !== "Все ингрeдиенты"){ //если есть фильтр
                        const header = document.createElement("h2");
                ...
                case "Сгруппировать по кол-ву ингредиентов": {
                    ...
                    Array.from(Object.keys(groups)).forEach(key => {
                        ...
                        if(Array.from(forTable.children).every(el => el.style.display === "none")){ //если в группе все не подходят под фильтр по ингредиенту
                    ...
                    const grids = document.querySelectorAll(".forTable");
                    Array.from(grids).forEach(div => { //удаляем пустые
                        if(div.children.length === 0) {div.remove();}
                    })
                    return;
                }

                case "Сгруппировать по кол-ву шагов": { //все то же самое, что и выше, но по шагам
...
function addListenerIng(div){ 
        div.addEventListener("click", (e) => {
            ...
            cards.forEach(card => { //если карточка рецепта из отфильтрованных
            ...
            Array.from(grids).forEach(table => {
                if(Array.from(table.children).every(el => el.style.display === "none")){ //если нет отображаемых карточек
...
function checkIngrFilter(div){
    ...
    if(ingredients.every(li => li.querySelector("p").textContent !== filtering)){
